...
### 解题思路
构建一个递减队列，队头是队列最大元素  
当插入一个元素时，由于是递减，从后向前插，并把小于插入元素的值全部删除  

这样做的原因是：   
新插入的的元素比你大，还比你后入队列，那之前所有小于新插元素的值没必要保存了，最大值肯定不是他们

那如果先入队列中，有比新插入元素大的值，这个肯定是在对首，当数组移动到这个值在数组中的位置时候，肯定是要删除它的。

1. push（val）：先将队列里面小于val的数都删掉，然后将val插入到队列；
2. pop（val）：用于删除过时的最大值，如果最大值没有过时那么它不会删除任何值
3. max（）：返回队列的第一个元素，也就是最大值


分析[1,3,-1,-3,5,3,6,7] 构造成的队列：
...
class maxQueue{
    ...
    push(x) {
        while(this.queue.length > 0 && x > this.queue[this.queue.length-1]) {
            this.queue.pop()
        }
        this.queue.push(x);
    }
...
### 9. [71. 简化路径](https://leetcode-cn.com/problems/simplify-path/)
...
### 10.  [150. 逆波兰表达式求值](https://leetcode-cn.com/problems/evaluate-reverse-polish-notation/)
...
如果遇到 运算符，把运算符和栈顶两个提出来，做运算后的结果放入 栈中

卡住在：

比如取得，操作符，"+" ， 但显然是个字符串，怎么变成操作符？

"2"  "+"  "4"

结果：

​	看了别人写的，可以把这个 变成方法，如果碰到这几个运算符，方法里面写具体的 操作符
